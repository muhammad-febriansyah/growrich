...
/** Props to trigger animation when element scrolls into view */
const inView = {
...
const defaultImage = 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2940&auto=format&fit=crop';
...
function SectionHeader({
    ...
    return (
        <motion.div {...inView} variants={stagger} className="mb-16 text-center space-y-4">
            <motion.div
                variants={fadeDown}
        ...
        <HomeLayout transparentNavbar>
                    ...
                    >
                        {features.map((feature, i) => (
                            ...
                            >
                                ...
                                <h3 className="relative mb-2 text-base font-bold text-gray-900">{feature.title}</h3>
                    ...
                    >
                        {packages.map((pkg) => {
                            ...
                            return (
                                <motion.div
                                    key={pkg.name}
                                    variants={pkg.is_popular ? scaleUp : fadeUp}
            ...
            <section id="marketing-plan" className="relative overflow-hidden bg-gray-50/50 py-24 lg:py-32">
                ...
                <div className="relative mx-auto max-w-6xl px-4 md:px-6">
                    ...
                    {/* Daily Bonuses */}
                    <motion.div {...inView} variants={stagger} className="mb-6">
                    ...
                    {/* Monthly Bonuses */}
                    <motion.div {...inView} variants={stagger} className="mb-16">
                        <motion.div variants={fadeUp} className="mb-4 flex items-center gap-3">
            ...
            <section className="relative overflow-hidden bg-gray-50/50 py-24 lg:py-32">
                ...
                <div className="relative mx-auto max-w-5xl px-4 md:px-6">
                    ...
                    <div className="relative">
                        ...
                        <motion.div {...inView} variants={staggerSlow} className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                            {[
                                { step: '01', icon: <LucideIcons.UserPlus className="h-6 w-6" />, title: 'Buat Akun', desc: 'Isi formulir pendaftaran dengan data diri yang valid. Proses hanya butuh 2 menit.', color: 'bg-primary/10 text-primary', dot: 'bg-primary' },
                                { step: '02', icon: <LucideIcons.Package className="h-6 w-6" />, title: 'Pilih Paket', desc: 'Tentukan paket Silver, Gold, atau Platinum sesuai target dan kemampuan Anda.', color: 'bg-emerald-50 text-emerald-600', dot: 'bg-emerald-500' },
                                { step: '03', icon: <LucideIcons.KeyRound className="h-6 w-6" />, title: 'Aktivasi PIN', desc: 'Masukkan PIN aktivasi yang didapat dari sponsor atau beli langsung melalui sistem.', color: 'bg-violet-50 text-violet-600', dot: 'bg-violet-500' },
                                { step: '04', icon: <LucideIcons.TrendingUp className="h-6 w-6" />, title: 'Raih Bonus', desc: 'Akun aktif! Mulai bangun jaringan dan nikmati berbagai bonus yang mengalir setiap hari.', color: 'bg-amber-50 text-amber-600', dot: 'bg-amber-500' },
                            ].map((s, i) => (
                                <motion.div
                                    key={i}
                                    variants={fadeUp}
            ...
            {testimonials.length > 0 && (
                <section className="relative overflow-hidden bg-white py-24 lg:py-32">
                    ...
                    <div className="relative mx-auto max-w-6xl px-4 md:px-6">
                        <SectionHeader
                            icon={<LucideIcons.MessageSquareQuote className="h-3.5 w-3.5" />}
                            badge="Testimoni Member"
                            title="Mereka Sudah"
                            highlight="Membuktikannya"
                            description="Ribuan member aktif telah merasakan manfaat nyata dari platform GrowRich."
                        />
                    </div>
            ...
            {faqs.length > 0 && (
                <section id="faq" className="relative overflow-hidden bg-gray-50/50 py-24 lg:py-32">
                    ...
                    <div className="relative mx-auto max-w-3xl px-4 md:px-6">
                        <SectionHeader
                            icon={<LucideIcons.HelpCircle className="h-3.5 w-3.5" />}
                            badge="FAQ"
                            title="Pertanyaan yang Sering"
                            highlight="Ditanyakan"
                            description="Temukan jawaban atas pertanyaan umum seputar GrowRich di sini."
                        />

                        <motion.div {...inView} variants={staggerSlow} className="space-y-3">
                            {faqs.map((faq, i) => (
                                <motion.div key={faq.id} variants={fadeUp}>
                                    <FaqAccordion faq={faq} defaultOpen={i === 0} />
                                </motion.div>
                            ))}
                        </motion.div>
...
function BonusCard({
    ...
    return (
        <div className="group relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-primary/15 hover:shadow-lg h-full">
            ...
            <div className="mb-4 flex items-start justify-between">
                <div className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${iconColor}`}>{icon}</div>
...
function FaqAccordion({ faq, defaultOpen = false }: { faq: FaqItem; defaultOpen?: boolean }) {
    ...
    return (
        <div className={`rounded-2xl border bg-white transition-colors duration-200 ${open ? 'border-primary/20 shadow-sm' : 'border-gray-100'}`}>
            <button
            ...
            >
                ...
                <motion.span
                    ...
                    className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${open ? 'bg-primary text-white' : 'bg-gray-100 text-gray-500'}`}
                >
